...
try {
    switch ($action) {
        ...
        case 'recent':
            $limit = validateInt(requestValue('limit', 10), 1, 50) ?? 10;
            $stmt = $pdo->prepare('SELECT order_no, status, delivery_status, price, created_at FROM orders ORDER BY id DESC LIMIT ?');
            ...
            if (commerceTableExists($pdo, 'tickets')) {
                $stmt = $pdo->prepare('SELECT id, title, status, category, updated_at FROM tickets ORDER BY id DESC LIMIT ?');
                $stmt->bindValue(1, $limit, PDO::PARAM_INT);
                $stmt->execute();
            ...
            if (commerceTableExists($pdo, 'credit_transactions')) {
                $stmt = $pdo->prepare('SELECT id, user_id, type, amount, created_at FROM credit_transactions ORDER BY id DESC LIMIT ?');
                $stmt->bindValue(1, $limit, PDO::PARAM_INT);
                $stmt->execute();
