...
function oauthConfig(): array {
    return [
        ...
        'auth_url' => defined('LINUXDO_AUTH_URL') ? LINUXDO_AUTH_URL : 'https://connect.linux.do/oauth2/authorize',
        'token_url' => defined('LINUXDO_TOKEN_URL') ? LINUXDO_TOKEN_URL : 'https://connect.linux.do/oauth2/token',
        'user_url' => defined('LINUXDO_USER_URL') ? LINUXDO_USER_URL : 'https://connect.linux.do/api/user',
    ];
}
...
switch ($action) {
    case 'login':
        ...
        header('Location: ' . $oauth['auth_url'] . '?' . $params);
        exit;
    ...
    case 'callback':
        ...
        $oauth = oauthConfig();
        $tokenData = getAccessToken($code, $oauth);
        if (!$tokenData || !isset($tokenData['access_token'])) {
            outputError('获取访问令牌失败');
        }

        $userInfo = getUserInfo($tokenData['access_token'], $oauth);
    ...
    case 'check':
        $oauth = oauthConfig();
        $configured = $oauth['client_id'] !== '' && $oauth['client_secret'] !== '' && $oauth['redirect_uri'] !== '';
...
function getAccessToken(string $code, array $oauth): ?array {
    $data = [
        'client_id' => $oauth['client_id'],
    ...
    $response = httpRequest($oauth['token_url'], [
        'method' => 'POST',
        'data' => $data,
        'headers' => [
...
function getUserInfo(string $accessToken, array $oauth): ?array {
    $response = httpRequest($oauth['user_url'], [
        'method' => 'GET',
        'headers' => [
