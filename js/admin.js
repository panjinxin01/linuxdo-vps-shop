...
function loadOAuthSettings() {
  apiFetch("../api/settings.php?action=get_oauth").then((r) => r.json()).then((data) => {
    ...
    const d = data.data;
    if (document.getElementById("cfgOAuthClientId")) document.getElementById("cfgOAuthClientId").value = d.client_id || "";
    if (document.getElementById("cfgOAuthClientSecretStatus")) document.getElementById("cfgOAuthClientSecretStatus").value = d.client_secret_masked || "未配置";
    if (document.getElementById("cfgOAuthRedirectUri")) document.getElementById("cfgOAuthRedirectUri").value = d.redirect_uri || "";
    if (document.getElementById("cfgOAuthConfigHint")) {
      document.getElementById("cfgOAuthConfigHint").innerHTML = (d.configured ? "当前已通过私有配置启用 OAuth。" : "当前尚未完成 OAuth 私有配置。") + " 请在服务器端通过 <code>/api/config.local.php</code> 或环境变量设置 <code>LINUXDO_CLIENT_ID</code>、<code>LINUXDO_CLIENT_SECRET</code>、<code>LINUXDO_REDIRECT_URI</code>。";
    }
  }).catch(() => {});
}
function saveOAuthSettings() {
  alert("OAuth 登录配置已改为私有配置模式。\n\n请在服务器端通过 /api/config.local.php 或环境变量设置：\n- LINUXDO_CLIENT_ID\n- LINUXDO_CLIENT_SECRET\n- LINUXDO_REDIRECT_URI\n\n前端后台不再直接保存 Client Secret。");
}
