...
function updateAdminUI() {
  ...
  if (avatar && currentAdminInfo.username)
    avatar.textContent = currentAdminInfo.username.charAt(0).toUpperCase();
  if (name) name.textContent = currentAdminInfo.username || "管理员";
  if (role)
    role.textContent =
      currentAdminInfo.role === "super" ? "超级管理员" : "管理员";
}
  ...
  apiFetch("../api/update_db.php?action=check")
    ...
    .then((data) => {
      if (
        data.code === 1 &&
        data.data &&
        !data.data.all_installed &&
        data.data.missing &&
        data.data.missing.length > 0
      ) {
        var el = document.getElementById("dbMissingTables");
        if (el)
          el.innerHTML = data.data.missing
            .map(
              (t) =>
                '<code style="background:rgba(0,0,0,0.2);padding:2px 8px;border-radius:4px;margin:2px 4px;display:inline-block">' +
                escapeHtml(t) +
                "</code>",
            )
            .join(" ");
...
function loadOrders(page = 1) {
  ...
  apiFetch(
    "../api/orders.php?action=all&page=" +
      page +
      "&page_size=" +
      adminOrderPagination.pageSize,
  )
    .then((r) => r.json())
    .then((data) => {
      ...
      tbody.innerHTML = list
        .map((o) => {
          ...
          let actionHtml = `<div class="admin-inline-actions"><button class="action-btn edit" onclick="showOrderDetail('${escapeHtml(o.order_no)}')">查看</button>`;
          if (numericStatus === 1) {
            actionHtml += `<button class="action-btn del" onclick="refundOrder('${escapeHtml(o.order_no)}', ${parseFloat(o.price || 0).toFixed(2)})">退款</button>`;
          }
          if (numericStatus !== 1) {
          ...
          return `<tr>
                    ...
                    <td>${escapeHtml(o.product_name || "商品已删除")}<div class="compact-meta">${deliveryText}</div></td>
                    <td>${escapeHtml(o.username || "-")}</td>
                    <td>${parseFloat(o.price || 0).toFixed(2)}<div class="compact-meta">${escapeHtml(o.payment_method || "-")}</div></td>
                    <td><span class="badge ${numericStatus === 1 ? "on" : numericStatus === 0 ? "wait" : "off"}">${payText}</span></td>
      ...
      const container = document
        ...
        ?.closest(".table-wrapper")?.parentNode;
      if (container && data.data.total_pages > 1) {
...
function showOrderDetail(orderNo) {
  apiFetch(
    "../api/orders.php?action=detail&order_no=" + encodeURIComponent(orderNo),
  )
    .then((r) => r.json())
    .then((data) => {
      ...
      const o = data.data;
      const payText =
        ["待支付", "已支付", "已退款", "已取消"][parseInt(o.status || 0)] ||
        "未知";
      const deliveryText = escapeHtml(
        o.delivery_status_text || o.delivery_status || "-",
      );
      ...
      const canRefund =
        parseInt(o.status || 0) === 1 &&
        !["refunded", "cancelled"].includes(String(o.delivery_status || ""));
      ...
      document.getElementById("adminTicketBody").innerHTML = `
                <div style="display:grid;gap:12px">
                    ...
                    <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border)">
                        <div class="form-group"><label>交付状态</label><select id="orderDeliveryStatus">${Object.keys(
                          statuses,
                        )
                          .map(
                            (k) =>
                              `<option value="${k}" ${o.delivery_status === k ? "selected" : ""}>${statuses[k]}</option>`,
                          )
                          .join("")}</select></div>
                        ...
                        <div class="admin-inline-actions">
                            ...
                            ${canRefund ? `<button class="btn btn-danger" onclick="refundOrder('${escapeHtml(o.order_no)}', ${parseFloat(o.price || 0).toFixed(2)})">立即退款</button>` : ""}
                        </div>
                    </div>
                </div>`;
      document.getElementById("adminTicketFoot").innerHTML =
        `<button class="btn btn-primary" onclick="closeAdminTicketDetail()">关闭</button>`;
...
balance = 退回站内余额`,
  ...
  const mode =
    String(choice).trim().toLowerCase() === "balance" ? "balance" : "original";
  const reason =
    prompt("请输入退款原因（会留痕记录）", "人工退款") || "人工退款";
  const body = new FormData();
...
function loadOAuthSettings() {
  apiFetch("../api/settings.php?action=get_oauth")
    ...
    .then((data) => {
      if (data.code === 1 && data.data) {
        document.getElementById("cfgOAuthClientId").value =
          data.data.client_id || "";
        document.getElementById("cfgOAuthClientSecret").value =
          data.data.client_secret || "";
        document.getElementById("cfgOAuthRedirectUri").value =
...
function migrateLinuxDOFields() {
  if (
    !confirm(
      "确定要执行数据库迁移吗？\n\n这将为users表添加Linux DO OAuth所需的字段。",
    )
  )
    return;
...
function loadSmtpSettings() {
  apiFetch("../api/settings.php?action=get_smtp")
    ...
    .then((data) => {
      if (data.code === 1 && data.data) {
        document.getElementById("cfgSmtpHost").value =
          data.data.smtp_host || "";
        ...
        document.getElementById("cfgSmtpPass").value =
          data.data.smtp_pass || "";
        document.getElementById("cfgSmtpFrom").value =
          data.data.smtp_from || "";
        document.getElementById("cfgSmtpName").value =
          data.data.smtp_name || "";
        document.getElementById("cfgSmtpSecure").value =
          data.data.smtp_secure || "tls";
      }
    })
    .catch(() => {});
}
    ...
    .then((data) => {
      if (data.code === 1 && data.data) {
        ...
        document.getElementById("cacheSize").textContent =
          data.data.size_human || "0 B";
        document.getElementById("cacheExpired").textContent =
          data.data.expired || 0;
      }
    })
    .catch(() => {});
...
function editProduct(p) {
  ...
  document.getElementById("pRiskReviewRequired").checked =
    parseInt(p.risk_review_required || 0) === 1;
  document.getElementById("pAllowWhitelistOnly").checked =
    parseInt(p.allow_whitelist_only || 0) === 1;
...
function saveProduct() {
  ...
  body.append(
    ...
    document.getElementById("pRiskReviewRequired").checked ? "1" : "0",
  );
  body.append(
    "allow_whitelist_only",
    document.getElementById("pAllowWhitelistOnly").checked ? "1" : "0",
  );
  ...
  apiFetch("../api/coupons.php?action=all")
    ...
    .then((data) => {
      ...
      if (
        ...
        tbody.innerHTML =
          '<tr><td colspan="8" class="empty">暂无优惠券</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = data.data.list
        .map((c) => {
          const isExpired = c.ends_at && new Date(c.ends_at) < new Date();
          const statusClass =
            c.status == 1 ? (isExpired ? "wait" : "on") : "off";
          const statusText =
            c.status == 1 ? (isExpired ? "已过期" : "有效") : "停用";
          const typeText = c.type === "fixed" ? "减免" : "折扣";
          const valueText =
            c.type === "fixed" ? c.value + "积分" : c.value + "%";
...
function editCouponById(id) {
  ...
  if (c.starts_at)
    document.getElementById("cStartsAt").value = c.starts_at
      .replace(" ", "T")
      .substring(0, 16);
  else document.getElementById("cStartsAt").value = "";

  if (c.ends_at)
    document.getElementById("cEndsAt").value = c.ends_at
      .replace(" ", "T")
      .substring(0, 16);
...
function loadTicketStats() {
  apiFetch("../api/tickets.php?action=stats")
    ...
    .then((data) => {
      if (data.code === 1) {
        document.getElementById("statTicketPending").textContent =
          data.data.pending;
        document.getElementById("statTicketReplied").textContent =
          data.data.replied;
        document.getElementById("statTicketClosed").textContent =
          data.data.closed;
        document.getElementById("statTicketTotal").textContent =
          data.data.total;
      }
    });
}
...
function showAdminTicketDetail(id) {
  Promise.all([
    ...
    const targetText =
      ticket.refund_target === "balance"
        ? "退回站内余额"
        : ticket.refund_target === "original"
          ? "原路退回"
          : "-";
    const orderInfo = ticket.order_info || null;
    document.getElementById("adminTicketTitle").textContent =
        ...
        (r) => `
            <div class="ticket-reply ${r.user_id ? "user" : "admin"}">
                <div class="reply-header">
                    <span class="reply-author">${r.user_id ? escapeHtml(r.username || "用户") : "客服 / 管理员"}</span>
    ...
    if (attachments.length > 0) {
      attachHtml = `
                <div class="ticket-attachments">
                    ...
                    <div class="ticket-attachments-grid">${attachments
                      .map((a) => {
                        const isImage = (a.mime_type || "").startsWith(
                          "image/",
                        );
                        const fileUrl = `../api/upload.php?action=download&id=${a.id}`;
                        const fileName = escapeHtml(a.original_name || "附件");
                        const fileMeta = a.file_size
                          ? `(${formatFileSize(a.file_size)})`
                          : "";
                        return isImage
                          ? `<a class="ticket-attachment image" href="${fileUrl}" target="_blank"><img src="${fileUrl}" alt="${fileName}"><span class="ticket-attachment-name">${fileName}</span><span class="ticket-attachment-meta">${fileMeta}</span></a>`
                          : `<a class="ticket-attachment file" href="${fileUrl}" target="_blank"><span class="ticket-attachment-icon">📄</span><span class="ticket-attachment-name">${fileName}</span><span class="ticket-attachment-meta">${fileMeta}</span></a>`;
                      })
                      .join("")}</div>
                </div>`;
    }
        ...
        ? `
            <div class="refund-admin-card">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
                    <div>
                        <div style="font-size:16px;font-weight:700;color:var(--text-main)">退款审批</div>
                        <div style="font-size:12px;color:var(--text-muted);margin-top:6px">用户提交时选择：${escapeHtml(targetText)}</div>
                    </div>
                    ${orderInfo ? `<span class="badge ${String(orderInfo.delivery_status || "") === "refunded" ? "off" : "wait"}">订单状态：${escapeHtml(["待支付", "已支付", "已退款", "已取消"][parseInt(orderInfo.status || 0)] || "-")} / ${escapeHtml(orderInfo.delivery_status || "-")}</span>` : ""}
                </div>
                <div class="refund-admin-grid">
                    <div class="refund-admin-item"><label>关联订单</label><div>${escapeHtml(ticket.order_no || "-")}</div></div>
                    <div class="refund-admin-item"><label>退款金额</label><div>${orderInfo ? parseFloat(orderInfo.price || 0).toFixed(2) + " 积分" : "-"}</div></div>
                    <div class="refund-admin-item"><label>提交原因</label><div>${escapeHtml(ticket.refund_reason || "-")}</div></div>
                    <div class="refund-admin-item"><label>处理管理员</label><div>${escapeHtml(ticket.handled_admin_name || "-")}</div></div>
                </div>
                ${
                  ticket.status != 2
                    ? `
                <div class="refund-admin-grid">
                    <div class="refund-admin-item">
                        <label>审批退款方式</label>
                        <select id="approveRefundTarget">
                            <option value="original" ${ticket.refund_target === "original" ? "selected" : ""}>原路退回</option>
                            <option value="balance" ${ticket.refund_target === "balance" ? "selected" : ""}>退回站内余额</option>
                        </select>
                    </div>
                    <div class="refund-admin-item" style="grid-column:1/-1">
                        <label>审批备注 / 退款原因</label>
                        <textarea id="approveRefundReason" rows="3" placeholder="会写入退款记录与工单回复">${escapeHtml(ticket.refund_reason || "工单退款")}</textarea>
                    </div>
                </div>`
                    : ""
                }
            ...
            <div style="margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid var(--border)">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
                    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
                        ...
                        ${ticket.order_no ? `<span style="color:var(--text-muted);font-size:13px">订单：${escapeHtml(ticket.order_no)}</span>` : ""}
                        <span style="color:var(--text-muted);font-size:13px">分类：${escapeHtml(ticket.category || "other")}</span>
                    </div>
                    <div style="font-size:12px;color:var(--text-muted)">更新时间：${escapeHtml(ticket.updated_at || "")}</div>
                </div>
            </div>
            ${refundMeta}
              ...
              ticket.status != 2
                ...
                <div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--border)">
                    ...
                    <div style="margin-top:8px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                        ...
                        <button class="btn btn-outline" style="padding:6px 12px;font-size:12px" onclick="uploadTicketAttachment(${ticket.id})">上传附件</button>
                    </div>
                </div>`
                : ""
            }`;
    let footHtml = `<button class="btn btn-primary" onclick="closeAdminTicketDetail()">关闭</button>`;
    if (ticket.status != 2) {
      footHtml = `<div class="admin-inline-actions">${ticket.category === "refund_request" ? `<button class="btn btn-danger" onclick="approveRefundTicket(${ticket.id})">同意退款并自动处理</button>` : ""}<button class="btn btn-outline" onclick="adminCloseTicket(${ticket.id})">关闭工单</button><button class="btn btn-primary" onclick="adminReplyTicket(${ticket.id})">发送回复</button></div>`;
    }
...
function loadAnnouncements() {
  apiFetch("../api/announcements.php?action=all")
    ...
    .then((data) => {
      ...
      if (data.code !== 1 || !data.data || data.data.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" class="empty">暂无公告</td></tr>';
        return;
      }
      announcementCache = {};
      data.data.forEach((a) => {
        announcementCache[a.id] = a;
      });
...
function editAnnouncementById(id) {
  ...
  document.getElementById("annPublishAt").value = a.publish_at
    ? a.publish_at.replace(" ", "T").substring(0, 16)
    : "";
  document.getElementById("annExpiresAt").value = a.expires_at
    ? a.expires_at.replace(" ", "T").substring(0, 16)
    : "";
  document.getElementById("announcementModal").classList.add("show");
}
    ...
    .then((data) => {
      if (data.code === 1) {
        ...
        if (info.all_installed) {
          ...
          statusDiv.innerHTML =
            '✅ 数据库状态正常，所有表已安装<br><small style="opacity:0.7">已安装: ' +
            info.existing.join(", ") +
            "</small>";
        } else {
          ...
          statusDiv.innerHTML =
            "⚠️ 发现缺失的表:<strong>" +
            info.missing.join(", ") +
            '</strong><br><small style="opacity:0.7">请点击"更新数据库"按钮进行安装</small>';
        }
      } else {
      ...
      if (data.code === 1) {
        ...
        if (data.data && data.data.created && data.data.created.length > 0) {
          statusDiv.innerHTML =
            "✅ " +
            data.msg +
            '<br><small style="opacity:0.7">新建表: ' +
            data.data.created.join(", ") +
            "</small>";
        } else {
...
function loadAdmins() {
  ...
  if (currentAdminInfo.role !== "super") {
    tbody.innerHTML =
      '<tr><td colspan="5" class="empty-tip">仅超级管理员可查看</td></tr>';
    const promoteTbody = document.getElementById("promoteUserTable");
    if (promoteTbody)
      promoteTbody.innerHTML =
        '<tr><td colspan="4" class="empty">仅超级管理员可提权</td></tr>';
    return;
  }
  apiFetch("../api/admin.php?action=list")
    ...
    .then((data) => {
      if (data.code !== 1 || !data.data) {
        tbody.innerHTML =
          '<tr><td colspan="5" class="empty-tip">暂无数据</td></tr>';
        return;
      }
      tbody.innerHTML = data.data
        .map(
          (admin) => `
                <tr>
                    ...
                    <td>${admin.role !== "super" ? `<button class="action-btn del" onclick="deleteAdmin(${admin.id})">删除</button>` : '<span class="compact-meta">-</span>'}</td>
                </tr>`,
        )
        .join("");
      loadPromotableUsers();
...
function loadAuditLogs(page = 1) {
  ...
  const container =
    document.getElementById("auditTableContainer") || tbody.parentNode;
  tbody.innerHTML = '<tr><td colspan="6" class="empty">加载中...</td></tr>';

  apiFetch(
    "../api/audit_logs.php?action=list&page=" +
      page +
      "&page_size=" +
  ...
  apiFetch("../api/templates.php?action=list")
    ...
    .then((data) => {
      ...
      if (
        ...
        tbody.innerHTML =
          '<tr><td colspan="5" class="empty">暂无模板</td></tr>';
        return;
      }
      tbody.innerHTML = data.data
        .map(
...
function saveTemplate() {
  ...
  [
    ...
    "memory:tplMemory",
    "disk:tplDisk",
    "bandwidth:tplBandwidth",
    "region:tplRegion",
    "line_type:tplLineType",
    "os_type:tplOsType",
    "description:tplDescription",
    "extra_info:tplExtraInfo",
    "sort_order:tplSort",
  ].forEach((pair) => {
  ...
  apiFetch("../api/templates.php", { method: "POST", body })
    ...
    .then((data) => {
      if (data.code === 1) {
        ...
        loadTemplateList();
        loadProductTemplateOptions();
      } else alert(data.msg || "生成失败");
    });
}

function loadCreditAdminUsers() {
  const keyword = document.getElementById("creditSearchKeyword")
    ? document.getElementById("creditSearchKeyword").value.trim()
    : "";
  apiFetch(
    "../api/credits.php?action=admin_users&keyword=" +
    ...
    .then((data) => {
      ...
      if (!tbody) return;
      const list =
        data && data.code === 1 && data.data && Array.isArray(data.data.list)
          ? data.data.list
          : [];
      if (!list.length) {
        tbody.innerHTML =
...
function loadCommunityOverview() {
  apiFetch("../api/community.php?action=overview")
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1 && data.data && data.data.settings) {
        document.getElementById("communitySilencedMode").value =
          data.data.settings.linuxdo_silenced_order_mode || "review";
      }
    });
  apiFetch("../api/community.php?action=rules")
    .then((r) => r.json())
    .then((data) => {
      ...
      if (
        ...
        tbody.innerHTML =
          '<tr><td colspan="5" class="empty">暂无规则</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = data.data
        .map(
          (r) =>
            `<tr><td>${r.id}</td><td>${escapeHtml(r.rule_type)}</td><td>${r.product_id ? "商品#" + r.product_id : "全局"} / ${r.linuxdo_id ? "LD#" + r.linuxdo_id : "本站#" + (r.user_id || "-")}</td><td>${escapeHtml(r.remark || "-")}</td><td><button class="action-btn edit" onclick="fillCommunityRule(${r.id})">编辑</button><button class="action-btn del" onclick="deleteCommunityRule(${r.id})">删除</button></td></tr>`,
        )
        .join("");
    });
  apiFetch("../api/community.php?action=discounts")
    .then((r) => r.json())
    .then((data) => {
      ...
      if (
        ...
        tbody.innerHTML =
          '<tr><td colspan="6" class="empty">暂无折扣规则</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = data.data
        .map(
          (r) =>
            `<tr><td>${r.id}</td><td>TL${r.trust_level}</td><td>${r.product_id ? "#" + r.product_id : "全局"}</td><td>${escapeHtml(r.discount_type)}</td><td>${parseFloat(r.discount_value || 0).toFixed(2)}</td><td><button class="action-btn edit" onclick="fillCommunityDiscount(${r.id})">编辑</button><button class="action-btn del" onclick="deleteCommunityDiscount(${r.id})">删除</button></td></tr>`,
        )
        .join("");
    });
}
...
function saveCommunitySettings() {
  ...
  body.append(
    "linuxdo_silenced_order_mode",
    document.getElementById("communitySilencedMode").value,
  );
  apiFetch("../api/community.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1) showToast("设置已保存");
...
function saveCommunityRule() {
  ...
  if (document.getElementById("ruleId").value)
    body.append("id", document.getElementById("ruleId").value);
  [
    "rule_type:ruleType",
    "product_id:ruleProductId",
    "user_id:ruleUserId",
    "linuxdo_id:ruleLinuxdoId",
    "remark:ruleRemark",
  ].forEach((pair) => {
    const [k, i] = pair.split(":");
    body.append(k, document.getElementById(i).value);
  });
  apiFetch("../api/community.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1) {
        ...
        document.getElementById("ruleType").value = "whitelist";
        loadCommunityOverview();
      } else alert(data.msg || "保存失败");
    });
}
...
function fillCommunityRule(id) {
  const r = window.__communityRules ? window.__communityRules[id] : null;
  if (!r) return;
...
function saveCommunityDiscount() {
  ...
  if (document.getElementById("discountId").value)
    body.append("id", document.getElementById("discountId").value);
  [
    "trust_level:discountTrustLevel",
  ...
  apiFetch("../api/community.php", { method: "POST", body })
    ...
    .then((data) => {
      if (data.code === 1) {
        ...
        document.getElementById("discountTrustLevel").value = "0";
        loadCommunityOverview();
      } else alert(data.msg || "保存失败");
    });
}
...
function fillCommunityDiscount(id) {
  const r = window.__communityDiscounts
    ? window.__communityDiscounts[id]
    : null;
  if (!r) return;
...
function deleteCommunityDiscount(id) {
  ...
  const body = new FormData();
  body.append("action", "delete_discount");
  body.append("id", id);
  apiFetch("../api/community.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
...
function loadReportDashboard() {
  Promise.all([
    ...
    apiFetch("../api/dashboard.php?action=ticket_summary").then((r) =>
      r.json(),
    ),
  ]).then(([summary, hotProducts, tickets]) => {
    if (summary.code === 1) {
      document.getElementById("reportTodayIncome").textContent = parseFloat(
        summary.data.today_income || 0,
      ).toFixed(2);
      document.getElementById("reportMonthIncome").textContent = parseFloat(
        summary.data.month_income || 0,
      ).toFixed(2);
      document.getElementById("reportBalanceOrders").textContent =
        summary.data.balance_paid_orders || 0;
      document.getElementById("reportEpayOrders").textContent =
        summary.data.epay_paid_orders || 0;
      document.getElementById("reportExceptionOrders").textContent =
        summary.data.exception_orders || 0;
      document.getElementById("reportBalanceTotal").textContent = parseFloat(
        summary.data.user_balance_total || 0,
      ).toFixed(2);
    }
    const hotEl = document.getElementById("reportHotProducts");
    if (hotEl) {
      hotEl.innerHTML =
        hotProducts.code === 1 && hotProducts.data.length
          ? hotProducts.data
              .map(
                (item) =>
                  `<tr><td>${escapeHtml(item.name)}</td><td>${item.order_count}</td><td>${item.paid_count}</td><td>${parseFloat(item.income || 0).toFixed(2)}</td></tr>`,
              )
              .join("")
    ...
    if (ticketEl) {
      ticketEl.innerHTML =
        tickets.code === 1 &&
        tickets.data.category_breakdown &&
        tickets.data.category_breakdown.length
          ? tickets.data.category_breakdown
              .map(
                (item) =>
                  `<tr><td>${escapeHtml(item.category)}</td><td>${item.total}</td></tr>`,
              )
              .join("")
...
function loadNotificationSettings() {
  apiFetch("../api/settings.php?action=get_notification")
    ...
    .then((data) => {
      ...
      const d = data.data;
      if (document.getElementById("cfgNotifEmail"))
        document.getElementById("cfgNotifEmail").checked =
          parseInt(d.notification_email_enabled || 0) === 1;
      if (document.getElementById("cfgNotifWebhook"))
        document.getElementById("cfgNotifWebhook").checked =
          parseInt(d.notification_webhook_enabled || 0) === 1;
      if (document.getElementById("cfgNotifWebhookUrl"))
        document.getElementById("cfgNotifWebhookUrl").value =
          d.notification_webhook_url || "";
      if (document.getElementById("cfgSilencedOrderMode"))
        document.getElementById("cfgSilencedOrderMode").value =
          d.linuxdo_silenced_order_mode || "review";
    });
}
...
function saveNotificationSettings() {
  ...
  body.append(
    ...
    document.getElementById("cfgNotifEmail").checked ? "1" : "0",
  );
  body.append(
    "notification_webhook_enabled",
    document.getElementById("cfgNotifWebhook").checked ? "1" : "0",
  );
  body.append(
  ...
  apiFetch("../api/settings.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1) showToast("通知配置已保存");
      else alert(data.msg || "保存失败");
    });
}
...
function saveOrderDeliveryStatus(orderNo) {
  ...
  const info = (
    document.getElementById("orderDeliveryInfo")?.value || ""
  ).trim();
  let status = statusEl ? statusEl.value : "paid_waiting";
  ...
  body.append(
    "delivery_note",
    document.getElementById("orderDeliveryNote")?.value || "",
  );
  body.append(
    "delivery_error",
    document.getElementById("orderDeliveryError")?.value || "",
  );
  apiFetch("../api/orders.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1) {
        showToast("交付状态已更新");
        loadOrders(adminOrderPagination.page || 1);
        showOrderDetail(orderNo);
      } else {
        alert(data.msg || "更新失败");
      }
...
function loadPromotableUsers() {
  const tbody = document.getElementById("promoteUserTable");
  if (!tbody) return;
  if (currentAdminInfo.role !== "super") {
    tbody.innerHTML =
      '<tr><td colspan="4" class="empty">仅超级管理员可提权</td></tr>';
  ...
  const keyword = (
    document.getElementById("promoteUserKeyword")?.value || ""
  ).trim();
  if (!keyword) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="empty">输入关键词后搜索</td></tr>';
    return;
  }
  tbody.innerHTML = '<tr><td colspan="4" class="empty">搜索中...</td></tr>';
  apiFetch(
    "../api/admin.php?action=search_users&keyword=" +
      encodeURIComponent(keyword) +
      "&limit=20",
  )
    .then((r) => r.json())
    .then((data) => {
      const list = data.code === 1 && Array.isArray(data.data) ? data.data : [];
      if (!list.length) {
        tbody.innerHTML =
          '<tr><td colspan="4" class="empty">未找到匹配用户</td></tr>';
        return;
      }
      tbody.innerHTML = list
        .map((u) => {
          const status = u.admin_id
            ? `<span class="badge on">已是${u.admin_role === "super" ? "超管" : "管理员"}</span>`
            : `<span class="badge wait">${u.has_password ? "可直接沿用密码" : "需单独设密码"}</span>`;
          const action = u.admin_id
            ? '<span class="compact-meta">无需提权</span>'
            : `<button class="action-btn edit" onclick="promoteExistingUser(${parseInt(u.id || 0)}, '${escapeHtml(u.username || "")}')">提权</button>`;
          return `<tr><td>${u.id}</td><td>${escapeHtml(u.username || "-")}<div class="compact-meta">${escapeHtml(u.email || u.linuxdo_username || "-")}</div></td><td>${status}</td><td>${action}</td></tr>`;
        })
        .join("");
    })
    .catch(() => {
      tbody.innerHTML = '<tr><td colspan="4" class="empty">搜索失败</td></tr>';
    });
}

function promoteExistingUser(userId, username) {
  const role = document.getElementById("promoteUserRole")?.value || "admin";
  const password = document.getElementById("promoteUserPassword")?.value || "";
  if (
    !confirm(
      `确定将用户 ${username} 提权为${role === "super" ? "超级管理员" : "普通管理员"}吗？`,
    )
  )
    return;
  apiFetch("../api/admin.php?action=promote_user", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    ...
    .then((data) => {
      if (data.code === 1) {
        showToast("提权成功");
        if (document.getElementById("promoteUserPassword"))
          document.getElementById("promoteUserPassword").value = "";
        loadAdmins();
        loadPromotableUsers();
      } else {
        showToast(data.msg || "提权失败", "error");
      }
    });
}
...
function clearAuditLogs() {
  if (currentAdminInfo.role !== "super") {
    showToast("仅超级管理员可清空日志", "error");
    return;
  }
  const password = prompt("请输入当前管理员密码，用于确认清空全部操作日志");
  ...
  apiFetch("../api/audit_logs.php", { method: "POST", body })
    .then((r) => r.json())
    .then((data) => {
      if (data.code === 1) {
        showToast("操作日志已清空");
...
function approveRefundTicket(ticketId) {
  if (!confirm("确认同意该退款申请并立即执行退款吗？")) return;
  const target =
    document.getElementById("approveRefundTarget")?.value || "original";
  const reason =
    (
      document.getElementById("approveRefundReason")?.value || "工单退款"
    ).trim() || "工单退款";
  const body = new FormData();
