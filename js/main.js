...
function renderUserArea() {
  ...
  if (currentUser) {
    ...
    if (area) area.innerHTML = `<div class="flex items-center gap-4"><span style="color:var(--text-light)">👤 ${escapeHtml(username)}</span>${currentRole === "user" ? `<span style="font-size:12px;color:var(--primary)">余额 ${balance.toFixed(2)}</span>` : ""}${adminBtn}<a href="#" class="nav-link" onclick="logout();return false;">退出</a></div>`;
...
function loadAnnouncements() {
  apiFetch("api/announcements.php?action=list").then((r) => r.json()).then((data) => {
    ...
    if (scrollList) scrollList.innerHTML = data.data.map((a) => `<div class="announcement-scroll-item" onclick="showAnnouncement(${a.id})"><div class="announcement-scroll-title">${a.is_top == 1 ? '<span class="tag">置顶</span>' : "🔔"} ${escapeHtml(a.title)}</div><div class="announcement-scroll-desc">${escapeHtml((a.content || "").substring(0, 100))}</div><a href="#" class="announcement-scroll-link" onclick="event.stopPropagation();showAnnouncement(${a.id})">详情及修复办法</a></div>`).join("");
  ...
  Promise.all([
    ...
    if (att.length > 0) {
      atHtml = `<div class="ticket-attachments"><div class="ticket-attachments-title">📎 附件 (${att.length})</div><div class="ticket-attachments-grid">` + att.map((a) => {
        ...
        return (a.mime_type || "").startsWith("image/") ? `<a class="ticket-attachment image" href="${u}" target="_blank"><img src="${u}" alt="${n}"><span class="ticket-attachment-name">${n}</span></a>` : `<a class="ticket-attachment file" href="${u}" target="_blank"><span class="ticket-attachment-icon">📄</span><span class="ticket-attachment-name">${n}</span></a>`;
      }).join("") + "</div></div>";
    }
